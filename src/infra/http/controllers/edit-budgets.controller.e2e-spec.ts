import { AppModule } from '@/app.module'
import { INestApplication } from '@nestjs/common'
import { describe, test, expect, beforeAll } from 'vitest'
import { Test } from '@nestjs/testing'
import request from 'supertest'
import { DatabaseModule } from '@/infra/database/database.module'
import { CategoryRepository } from '@/domain/repositories/category-repository'
import { Category } from '@/domain/entities/category'
import { Budget } from '@/domain/entities/budget'
import { BudgetRepository } from '@/domain/repositories/budget-repository'
import { UserRepository } from '@/domain/repositories/user-repository'
import { User } from '@/domain/entities/user'
import { JwtService } from '@nestjs/jwt'

describe('Edit Budgets [E2E]', () => {
  let app: INestApplication
  let jwt: JwtService
  let budgetRepository: BudgetRepository
  let userRepository: UserRepository
  let categoryRepository : CategoryRepository

  beforeAll(async () => {
    const moduleRef = await Test.createTestingModule({
      imports: [AppModule, DatabaseModule],
    }).compile()

    app = moduleRef.createNestApplication()

    jwt = moduleRef.get(JwtService)
    categoryRepository = moduleRef.get(CategoryRepository)
    budgetRepository = moduleRef.get(BudgetRepository)
    userRepository = moduleRef.get(UserRepository)
    await app.init()
  })

  test('shoud be able to edit categories', async () => {
    const user = new User({
      name: 'Jonh Doe',
      email: 'jonhdoe@example.com',
      password: 'password',
      createdAt: new Date(),
    })

    await userRepository.create(user)
    const accessToken = jwt.sign({ sub: user.id })

    const category = new Category({
      name: 'test',
      description: 'test category',
    })

    await categoryRepository.create(category)

    const budgets = new Budget({
      amount: 100,
      categoryId: category.id,
      startDate: new Date('2022-01-01'),
      endDate: new Date('2022-12-31'),
      userId: user.id,
      createdAt: new Date(),
    })

    await budgetRepository.create(budgets)

    const response = await request(app.getHttpServer())
      .put(`/budgets/${budgets.id}`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send({
        amount: 230,
        categoryId: category.id,
        startDate: new Date('2023-02-25'),
        endDate: new Date('2023-12-31'),
      })

    console.log(await budgetRepository.findById(budgets.id))
    expect(response.statusCode).toBe(204)
    expect(await budgetRepository.findById(budgets.id)).toMatchObject({
      amount: 230,
      categoryId: category.id,
      startDate: new Date('2023-02-25'),
      endDate: new Date('2023-12-31'),
    })
  })
})
